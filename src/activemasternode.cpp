...
//
// Bootup the masternode, look for a 6000 HER input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Hera.conf */
...
{
    CHeracoinAddress address(collateralAddress);
