...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeHeraAmount"))
        settings.setValue("nAnonymizeHeraAmount", 1000);
    nAnonymizeHeraAmount = settings.value("nAnonymizeHeraAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeHeraAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeHeraAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeHeraAmount:
            return QVariant(nAnonymizeHeraAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeHeraAmount:
            nAnonymizeHeraAmount = value.toInt();
            settings.setValue("nAnonymizeHeraAmount", nAnonymizeHeraAmount);
            emit AnonymizeHeraAmountChanged(nAnonymizeHeraAmount);
            break;
        default:
            break;
        }
