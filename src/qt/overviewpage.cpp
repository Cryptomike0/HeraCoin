...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeHeraAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHeraAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeHeraAmount = strAnonymizeHeraAmount.remove(strAnonymizeHeraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHeraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeHeraAmount*COIN) nMaxToAnonymize = nAnonymizeHeraAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeHeraAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeHeraAmount));
        strAnonymizeHeraAmount = strAnonymizeHeraAmount.remove(strAnonymizeHeraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHeraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
